...
echo Format: "[GeneratedFile] [srcDir(\`pwd\`)] [grooveWidth(1.2)] [grooveDepth(1.8)] [omegaGrooveGap(1.2/2)] [grooveOffset(3)] [Omega/Round/Full] [EJ/Air] [Single/Double] [fiberR] [optional: triangleBarL(500)] [optional: triangleBarN0(25)] [optional: coatingThickness(0.12)]"
echo Tips:
...
echo -n "        <volumeref ref=\"Detector-${shape:0:3}-${coupling}" >>$GeneratedFile

if [[ -n $single ]]; then
    echo $single
    if [[ "$single" == "Single" ]]; then
        echo -n "-Single" >>$GeneratedFile
    fi
fi
