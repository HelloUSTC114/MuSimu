...
void GdmlConstruction::Init()
{
	...
	if (!fTarget)
	{
		fTarget = lvStore->GetVolume("Matter", false);
	}
	...
	if (lvStore->GetVolume("Scintillator"))
		lvStore->GetVolume("Scintillator")->SetVisAttributes(scinVis);
	// worldVis->SetDaughtersInvisible(false);
	if (fDetector)
	...
	const G4GDMLAuxListType *auxList, G4String prefix)
{
		 ...
		 auxIter != auxList->end(); auxIter++)
	{
		...
		else if (auxIter->type == "Material")
		{
			...
			{
				G4cerr << "[#] ERROR - G4Material " << auxIter->value << " NOT FOUND" << G4endl;
				return false;
			}
		}
		else
